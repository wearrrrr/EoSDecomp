...
AsciiManager::make_popup_B(AsciiManager *this,Float3 *position,int number,D3DCOLOR color)

{
  int idx;
  AsciiManagerPopup *popup;
  
  if (2 < this->next_popupB_index) {
    this->next_popupB_index = 0;
  }
  popup = this->popups_2 + this->next_popupB_index;
  popup->in_use = 1;
  idx = 0;
  if (number < 0) {
    popup->digits[0] = '\n';
  ...
  this->next_popupB_index = this->next_popupB_index + 1;
  return this;
}
